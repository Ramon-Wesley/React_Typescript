...
export const Agendamentos: React.FC = () => {
  ...
  const [rows, setRows] = useState<IAgendamento[]>([]);
  ...
  const handleDelete = useCallback(() => {
    setOpen(false);
    if (idResult.current) {
      AgendamentoService.deleteById(idResult.current).then((response) => {
  ...
  useEffect(() => {
    ...
    debounce(() => {
      ...
      AgendamentoService.getAll(getSearch, getPage).then((response) => {
        setIsLoading(false);
        if (response instanceof Error) {
          setMessageAlert(response.message);
          setTypeAlert('error')
  ...
  return (
    <LayoutBase
      ...
      tools={
        <SearchTools
          textButton="Nova"
          onclickButton={()=>navigate('/agendamentos/detalhe/nova')}
          onchangeInput={(e) =>
            ...
            {rows.map((row) => (
              <TableRow key={row.id}>
                <TableCell>
                  ...
                  <IconButton
                    size="small"
                    onClick={() => navigate(`/agendamentos/detalhe/${row.id}`)}
                  >
                    <Icon>edit</Icon>
                  </IconButton>
                </TableCell>
                <TableCell>{row.funcionario?.nome}</TableCell>
                <TableCell>{row.animal?.cliente?.nome}</TableCell>
                <TableCell>{row.animal?.nome}</TableCell>
                <TableCell>{row.data.toString().split('T')[0]}</TableCell>
              </TableRow>
            ))}
          </TableBody>
          <TableFooter>
