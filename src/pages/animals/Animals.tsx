...
export const Animais: React.FC = () => {
  ...
  const [rows, setRows] = useState<IAnimals[]>([]);
  ...
  const handleDelete = useCallback(() => {
    setOpen(false);
    if (idResult.current) {
      animalService.deleteById(idResult.current).then((response) => {
  ...
  return (
    <LayoutBase
      ...
      tools={
        <SearchTools
          textButton="Nova"
          onclickButton={()=>navigate('/animais/detalhe/nova')}
          onchangeInput={(e) =>
            ...
            {rows.map((row) => (
              <TableRow key={row.id}>
                <TableCell>
                  ...
                  <IconButton
                    size="small"
                    onClick={() => navigate(`/animais/detalhe/${row.id}`)}
                  >
                    <Icon>edit</Icon>
                  </IconButton>
                </TableCell>
                ...
                <TableCell>{row.data_de_nascimento.toString()}</TableCell>
