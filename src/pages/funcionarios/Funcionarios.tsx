...
export const Funcionario: React.FC = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const idResult = useRef<number>();
  const navigate = useNavigate();
  const [rows, setRows] = useState<IFuncionarios[]>([]);
  ...
  const handleDelete = useCallback(() => {
    setOpen(false);
    if (idResult.current) {
      FuncionarioService.deleteById(idResult.current).then((response) => {
  ...
  return (
    <LayoutBase
      ...
      tools={
        <SearchTools
          textButton="Nova"
          onclickButton={()=>navigate('/funcionarios/detalhe/nova')}
          onchangeInput={(e) =>
