...
import { IServicos, ServicoService } from "../../shared/services/api/servico/ServicoService";
...
: React.FC = () => {
  ...
  const [rows, setRows] = useState<IServicos[]>([]);
  ...
  const handleDelete = useCallback(() => {
    setOpen(false);
    if (idResult.current) {
      ServicoService.deleteById(idResult.current).then((response) => {
  ...
  return (
    <LayoutBase
      ...
      tools={
        <SearchTools
          textButton="Nova"
          onclickButton={()=>navigate('/servicos/detalhe/nova')}
          onchangeInput={(e) =>
            ...
            {rows.map((row) => (
              <TableRow key={row.id}>
                <TableCell>
                  ...
                  <IconButton
                    size="small"
                    onClick={() => navigate(`/servicos/detalhe/${row.id}`)}
                  >
                    <Icon>edit</Icon>
                  </IconButton>
                </TableCell>
