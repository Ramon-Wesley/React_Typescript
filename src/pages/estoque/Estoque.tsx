...
: React.FC = () => {
  ...
  const [rows, setRows] = useState<IEstoques[]>([]);
  ...
  const handleDelete = useCallback(() => {
    setOpen(false);
    if (idResult.current) {
      EstoqueService.deleteById(idResult.current).then((response) => {
  ...
  return (
    <LayoutBase
      ...
      tools={
        <SearchTools
          textButton="Nova"
          onclickButton={()=>navigate('/estoques/detalhe/nova')}
          onchangeInput={(e) =>
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>EstoqueAbaixo</TableCell>
            ...
            {rows.map((row) => (
              <TableRow key={row.id}>
                <TableCell>
                  ...
                  <IconButton
                    size="small"
                    onClick={() => navigate(`/estoques/detalhe/${row.id}`)}
                  >
                    <Icon>edit</Icon>
                  </IconButton>
                </TableCell>
                ...
                <TableCell>{row.estoqueAbaixo}</TableCell>
