import {Box,Drawer,Divider,Avatar,useTheme,useMediaQuery,List,ListItemButton,ListItemIcon,Icon,ListItemText} from '@mui/material'
...
const {toggleTheme} = useThemeContextOption()
    return(
        <>
            <Drawer open={isOpen}  variant={smDown? 'temporary':'permanent'} onClose={handleToggleOpen}>
                <Box width={theme.spacing(28)} display='flex' flexDirection='column' height='100%'>
                    <Box display='flex' height={theme.spacing(18)} justifyContent='center' alignItems='center'>
                    <Avatar sx={{width:theme.spacing(10),height:theme.spacing(10) }}/>
                    </Box>
                <Divider/>
                <Box flex={1}>
                <List>
                ...
                <ListItemIcon><Icon>theme</Icon></ListItemIcon>
        ...
        <Box marginLeft={smDown?0:theme.spacing(28)} height='100vh'>{children}</Box>
        </>
    )
}